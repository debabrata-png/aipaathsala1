...
const AdvanceAIVideoAnalysisInterface = () => {
  ...
  const fetchScheduledClasses = async () => {
    setLoading(true);
    try {
      const response = await ep3.get("/monitorscheduledclassesadvance", {
        params: {
          colid: global1.colid,
  ...
  const deleteAnalysis = async (analysisId) => {
    ...
    try {
      const response = await ep3.delete(
        `/deleteaivideoanalysisadvance/${analysisId}`,
        {
          params: {
            colid: global1.colid,
            user: global1.userEmail,
      ...
      <Card>
        <CardContent>
          ...
          {analyses.length === 0 ? (
            <Alert severity="info">
              <Typography variant="body2">
                <strong>No analysis yet.</strong> Start an AI analysis above to
                see results here.
              </Typography>
            </Alert>
          ) : (
...
export default AdvanceAIVideoAnalysisInterface;
