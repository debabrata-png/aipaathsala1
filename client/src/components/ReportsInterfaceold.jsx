...
const ReportsInterface = () => {
  ...
  const generateSingleStudentReport = async (formData) => {
    ...
    setIsLoading(true);
    try {
      const response = await ep3.get('/getsinglestudentreport', {
        params: {
  ...
  const handleCourseReportFlow = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await ep3.get('/getfacultycourses', {
  ...
  const generateCourseReport = async (selectedCourse) => {
    ...
    setIsLoading(true);
    try {
      const response = await ep3.get('/getclassreportbycourse', {
        params: {
          user: global1.userEmail,
  ...
  const generateDateRangeReport = async (dateRange) => {
    ...
    setIsLoading(true);
    try {
      const response = await ep3.get('/getattendancesummarybydate', {
        params: {
          user: global1.userEmail,
          colid: global1.colid,
