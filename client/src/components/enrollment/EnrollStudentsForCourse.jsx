...
const EnrollStudentsForCourse = ({ course }) => {
  ...
  const loadLinks = async () => {
    if (!course) return;
    try {
      const r = await ep3.get("/enrollment-links", {
        params: {
          colid: global1.colid,
  ...
  const createLink = async () => {
    try {
      const r = await ep3.post("/create-enrollment-link", {
        colid: global1.colid,
        user: global1.userEmail,
      ...
      if (r.data.success) {
        const url = `${window.location.origin}/enroll/${r.data.data.token}`;
        setLink(url);
  ...
  const revoke = async (token) => {
    try {
      const r = await ep3.put(`/enrollment-link/${token}/revoke`);
      if (r.data.success) {
        setMsg("Link revoked");
        setErr("");
        loadLinks();
