...
const AssignmentRoom = ({ course, socket, roomId }) => {
  ...
  const fetchAssignments = async () => {
    ...
    try {
      ...
      if (permissions.isStudent) {
        response = await ep3.get('/getassignmentsfirstudent', {
          params: {
        ...
        response = await ep3.get('/getassignmentsbycourse', {
          params: {
            coursecode: course.coursecode,
            colid: global1.colid
          }
  ...
  const fetchSubmissions = async (assignmentId) => {
    ...
    const response = await ep3.get(`/getassignmentsubmissions/${assignmentId}`);
  ...
  const handleCreateAssignment = async () => {
    ...
    try {
      const response = await ep3.post('/createassignment', {
  ...
  const handleSubmitAssignment = async () => {
    ...
    try {
      const response = await ep3.post('/submitassignment', {
        name: global1.userName,
