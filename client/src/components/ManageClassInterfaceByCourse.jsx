...
import CreateClassInterface from "./CreateClassInterface";
...
const ManageClassesInterfaceByCourse = ({ course }) => {
  ...
  const fetchClasses = async () => {
    ...
    try {
      let list = [];
      if (isFaculty) {
        const response = await ep3.get("/getclassesbycourse", {
  ...
  const handleSaveAttendance = async () => {
    ...
    try {
      ...
      const response = await ep3.post("/markattendance", { attendanceRecords });
      if (response.data.success) {
  ...
  const handleDeleteConfirm = async () => {
    try {
      const response = await ep3.delete(
        `/deleteclass/${deleteDialog.class._id}`,
        {
  ...
  const handleManualCreateConfirm = async () => {
    ...
    try {
      ...
      const resp = await ep3.post("/createclass", payload);
