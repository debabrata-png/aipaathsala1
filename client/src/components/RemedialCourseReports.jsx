...
const RemedialCourseReports = () => {
  ...
  const loadReports = async () => {
    ...
    try {
      ...
      const [att, low, enr, asg, tst, ses, stu, enrt, asgBy, tstBy, syl, sylBy, mat, matBy, ai, col, colBy, lib, noti] = await Promise.all([
        getAttendanceSummary('remedial', params),
        getAttendanceBelow('remedial', { ...params, threshold: 75 }),
        getCourseSummary('remedial', params),
        getAssignmentsSummary('remedial', params),
        getTestsSummary('remedial', params),
        getAttendanceBySession('remedial', params),
        getAttendanceStudentTable('remedial', params),
        getCourseOverTime('remedial', params),
        getAssignmentsByAssignment('remedial', params),
        getTestsByTest('remedial', params),
        getSyllabusSummary('remedial', params),
        getSyllabusBySession('remedial', params),
        getMaterialsSummary('remedial', params),
        getMaterialsOverTime('remedial', params),
        getAiSummary('remedial', params),
        getCollabSummary('remedial', params),
        getCollabOverTime('remedial', params),
        getLibrarySummary('remedial', params),
        getNotificationsSummary('remedial', params),
      ]);
      setSummary({
  ...
  return (
    <Box sx={{ height: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: '#f5f5f5' }}>
      <Paper elevation={2} sx={{ p: 2, backgroundColor: '#7b1fa2', color: 'white' }}>
        <Typography variant="h5" sx={{ fontWeight: 600 }}>🛠️ Remedial Reports</Typography>
        <Typography variant="body2">KPIs, bands, and tables for remedial courses</Typography>
      </Paper>

      <Box sx={{ p: 2 }}>
        <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mb: 2 }}>
          {['attendance','courses','assignments','tests','syllabus','materials','ai','collab','library','notifications'].map(tab => (
...
export default RemedialCourseReports;
