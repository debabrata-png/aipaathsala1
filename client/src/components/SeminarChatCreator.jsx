// src/components/SeminarChatCreator.jsx
...
import { Send, Group, CheckCircle } from '@mui/icons-material';
import FileUpload from './FileUpload';
import ep3 from '../api/ep3';
import global1 from '../pages/global1';
import { ocrAndCheckValues } from '../utils/ocrService';

const SeminarChatCreator = () => {
  ...
  const seminarQuestions = [
    {
      field: 'title',
      question: 'What is the title/topic of the seminar?',
      type: 'text',
      required: true,
      placeholder: 'Enter seminar title or topic'
    },
    {
      field: 'type',
      question: 'What type of seminar is this?',
      type: 'select',
      required: true,
      options: ['Conference', 'Workshop', 'Symposium', 'Webinar', 'Training', 'Seminar', 'Other']
    },
    {
      field: 'duration',
    ...
    {
      field: 'yop',
      question: 'In which year did this seminar take place?',
      type: 'text',
      required: true,
    ...
    {
      field: 'role',
      question: 'What was your role in this seminar?',
      type: 'select',
      required: true,
      options: ['Participant', 'Presenter', 'Organizer', 'Keynote Speaker', 'Session Chair', 'Panelist', 'Other']
    },
    {
      field: 'level',
      question: 'What is the level of this seminar?',
      type: 'select',
      required: true,
      options: ['International', 'National', 'Regional', 'State', 'Local', 'Institutional']
    },
    {
      field: 'membership',
      question: 'What is your membership in the organizing body?',
      type: 'text',
      required: true,
    },
    {
      field: 'paper',
      question: 'What Paper you will present in the seminar? (Optional)',
      type: 'text',
      required: false,
    },
    {
      field: 'amount',
      question: 'What was the registration/participation fee? (Optional)',
      type: 'number',
      required: false,
    ...
    },
    {
      ...
      type: 'select',
      required: true,
      options: ['Completed', 'Upcoming', 'Ongoing', 'Cancelled', 'Postponed']
    },
    {
      field: 'comments',
      question: 'Any additional comments about this seminar? (Optional)',
      type: 'textarea',
      required: false,
      placeholder: 'Enter any additional details, learnings, or outcomes'
    }
  ];
  ...
  useEffect(() => {
    setMessages([{
      type: 'bot',
      content: `👋 Hello ${global1.userName}! I'll help you register a seminar/conference/workshop. Let's gather the necessary information.`,
      timestamp: new Date()
    }, {
      type: 'bot',
      content: seminarQuestions[0].question,
      timestamp: new Date()
    }]);
  }, []);
  ...
  const handleSendMessage = () => {
    ...
    const currentQuestion = seminarQuestions[currentStep];
  ...
  const moveToNextQuestion = () => {
    ...
    if (nextStep < seminarQuestions.length) {
      setCurrentStep(nextStep);
      setCurrentInput('');
      setMessages(prev => [...prev, {
        type: 'bot',
        content: seminarQuestions[nextStep].question,
        timestamp: new Date()
      }]);
    } else {
  ...
  const handleValidationChoice = (choice) => {
    ...
    if (choice === 'check') {
      ...
      setMessages(prev => [...prev, {
        type: 'bot',
        content: 'Select a document to check if it matches your entered data: name, seminar title, year, and your role.',
        timestamp: new Date()
      }]);
    } else {
  ...
  const handleDocumentCheck = async () => {
    ...
    // Create items to check: name, title, yop, role
    const itemsToCheck = [
      global1.userName,
      seminarData.title,
      seminarData.yop,
  ...
  const handleDocumentChoice = (choice) => {
    ...
    if (choice === 'upload') {
      ...
      setMessages(prev => [...prev, {
        type: 'bot',
        content: 'Great! All information collected. Ready to save your seminar.',
        timestamp: new Date()
      }]);
    }
  };
  ...
  const handleFileUpload = (fileUrl) => {
    setSeminarData(prev => ({ ...prev, doclink: fileUrl }));
  ...
  const handleLinkSubmit = () => {
    if (currentInput.trim()) {
      setSeminarData(prev => ({ ...prev, doclink: currentInput.trim() }));
      setMessages(prev => [...prev, {
        type: 'user',
        content: currentInput,
        timestamp: new Date()
      ...
      setMessages(prev => [...prev, {
        type: 'bot',
        content: 'Document link saved! All information collected. Ready to save your seminar.',
        timestamp: new Date()
      }]);
    }
  };
  ...
  const handleSelectOption = (value) => {
    const currentQuestion = seminarQuestions[currentStep];
    setSeminarData(prev => ({ ...prev, [currentQuestion.field]: value }));
  ...
  const handleSubmitSeminar = async () => {
    ...
    try {
      const payload = {
        ...
        yop: seminarData.yop || '',
        membership: seminarData.membership || '',
        amount: seminarData.amount || 0,
        status1: seminarData.status1 || '',
        comments: seminarData.comments || '',
        role: seminarData.role || '',
        paper: seminarData.paper || '',
        level: seminarData.level || '',
        doclink: seminarData.doclink || '',
        type: seminarData.type || ''
      };

      const response = await ep3.post('createseminar', payload);
      
      if (response?.data?.success) {
        setMessages(prev => [...prev, {
          type: 'bot',
          content: 'Seminar saved successfully! 🎉',
          timestamp: new Date()
        }]);
      } else {
        setMessages(prev => [...prev, {
          type: 'bot',
          content: 'Failed to save seminar. Please try again.',
          timestamp: new Date()
        }]);
      }
    } catch (error) {
      setMessages(prev => [...prev, {
        type: 'bot',
        content: 'Error saving seminar. Please try again.',
        timestamp: new Date()
      }]);
    } finally {
  ...
  const currentQuestion = seminarQuestions[currentStep];
  const allBasicQuestionsAnswered = currentStep >= seminarQuestions.length;
      ...
      <Box sx={{ p: 2, flexGrow: 1, overflowY: 'auto', backgroundColor: '#f8fafc' }}>
        ...
        {isCompleted && !showFileUpload && !showDocumentOptions && !showDocCheck && !showValidationChoice && !showLinkInput && (
          <Box sx={{ mt: 2, p: 2, border: '1px solid #059669', borderRadius: 1, backgroundColor: '#f0fdf4' }}>
            ...
            <Typography variant="h6" sx={{ mb: 2 }}>Seminar Summary:</Typography>
            <Box sx={{ display: 'grid', gap: 1 }}>
              <Typography><strong>Title:</strong> {seminarData.title}</Typography>
              <Typography><strong>Type:</strong> {seminarData.type}</Typography>
              <Typography><strong>Duration:</strong> {seminarData.duration}</Typography>
              <Typography><strong>Year:</strong> {seminarData.yop}</Typography>
              <Typography><strong>Role:</strong> {seminarData.role}</Typography>
              <Typography><strong>Level:</strong> {seminarData.level}</Typography>
              <Typography><strong>Status:</strong> {seminarData.status1}</Typography>
              {seminarData.membership && <Typography><strong>Membership:</strong> {seminarData.membership}</Typography>}
              {seminarData.paper && <Typography><strong>Paper Presented:</strong> {seminarData.paper}</Typography>}
              {seminarData.amount > 0 && <Typography><strong>Amount:</strong> ₹{seminarData.amount}</Typography>}
              {seminarData.comments && <Typography><strong>Comments:</strong> {seminarData.comments}</Typography>}
              {seminarData.doclink && <Typography><strong>Document:</strong> Added</Typography>}
            </Box>
            ...
            >
              {isSubmitting ? <CircularProgress size={20} /> : 'Save Seminar'}
            </Button>
          </Box>
        )}
      </Box>
...
export default SeminarChatCreator;
