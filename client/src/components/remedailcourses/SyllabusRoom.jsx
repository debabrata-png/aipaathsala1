...
const SyllabusRoom = ({ course, socket, roomId }) => {
  ...
  const fetchSyllabus = async () => {
    ...
    setLoading(true);
    try {
      const r = await ep3.get("/getsyllabusbycourseremedial", {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const r = await ep3.post("/createsyllabusremedial", {
  ...
  const handleCompleteToggle = async (syllabusId, currentStatus) => {
    ...
    try {
      ...
      const r = await ep3.put(`/marksyllabuscompleteremedial/${syllabusId}`, {
        completed: newStatus,
      });
      if (r.data.success) {
        fetchSyllabus();
