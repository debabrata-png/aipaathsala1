...
const SyllabusRoom = ({ course, socket, roomId }) => {
  ...
  const fetchSyllabus = async () => {
    ...
    setLoading(true);
    try {
      const r = await ep3.get("/getsyllabusbycourseadvance", {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const r = await ep3.post("/createsyllabusadvance", {
  ...
  const handleCompleteToggle = async (syllabusId, currentStatus) => {
    ...
    try {
      ...
      const r = await ep3.put(`/marksyllabuscompleteadvance/${syllabusId}`, {
        completed: newStatus,
      });
      if (r.data.success) {
        fetchSyllabus();
