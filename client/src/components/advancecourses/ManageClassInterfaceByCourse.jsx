...
const ManageClassesInterfaceByCourse = ({ course }) => {
  ...
  const fetchClasses = async () => {
    ...
    try {
      let list = [];
      if (isFaculty) {
        const response = await ep3.get("/getclassesbyuseradvance", {
          params: {
        ...
        const response = await ep3.get("/getclassesbycourseadvance", {
  ...
  const fetchAIAnalyses = async () => {
    if (!isFaculty) return;
    try {
      const response = await ep3.get("/getaivideoanalysisbyuseradvance", {
  ...
  const fetchEnrolledStudents = async (classItem) => {
    try {
      const response = await ep3.get("/getenrolledstudentsadvance", {
  ...
  const handleSaveAttendance = async () => {
    ...
    try {
      ...
      const response = await ep3.post("/markattendanceadvance", { attendanceRecords });
      if (response.data.success) {
  ...
  const handleEditSave = async () => {
    try {
      const response = await ep3.put(`/updateclassadvance/${editDialog.class._id}`, {
        ...editDialog.class,
        user: global1.userEmail,
        colid: global1.colid,
      });
  ...
  const handleDeleteConfirm = async () => {
    try {
      const response = await ep3.delete(
        `/deleteclassadvance/${deleteDialog.class._id}`,
        {
  ...
  const triggerAIAnalysis = async (classItem) => {
    ...
    try {
      const response = await ep3.post("/processaivideoanalysisadvance", {
        classid: classId,
        user: global1.userEmail,
        colid: global1.colid,
      });
  ...
  const handleManualCreateConfirm = async () => {
    ...
    try {
      ...
      const resp = await ep3.post("/createclassadvance", payload);
