...
const RemedialAiVideoAnalysisInterface = () => {
  ...
  const fetchScheduledClasses = async () => {
    setLoading(true);
    try {
      const response = await ep3.get("/monitorscheduledclassesremedial", {
        params: {
          colid: global1.colid,
  ...
  const deleteAnalysis = async (analysisId) => {
    ...
    try {
      const response = await ep3.delete(
        `/deleteaivideoanalysisremedial/${analysisId}`,
        {
          params: {
            colid: global1.colid,
            user: global1.userEmail,
...
export default RemedialAiVideoAnalysisInterface;
